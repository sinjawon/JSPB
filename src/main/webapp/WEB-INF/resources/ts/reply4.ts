...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let addReplyBtn4 = document.getElementById("addReplyBtn4") as HTMLButtonElement;
    if (addReplyBtn4) {
    ...
    document.addEventListener("click", (event) => {
        if (event.target && event.target.className == "deleteReply4") {
            let replyNum4 = parseInt((event.target as HTMLElement).getAttribute("data-replynum") || "", 10);
...
async function loadReply4() {
    try {
        let boardListNumInput4 = document.getElementById("boardListNumInput4") as HTMLInputElement;
        let boardListNum4 = boardListNumInput4.value;

        let formData = new FormData();
        formData.append("boardListNum4", boardListNum4);

        let response = await fetch(`/api/replylist4?${new URLSearchParams(formData)}`, {
            method: "GET",
            headers: {
...
function displayReply4(replies4: { replyList4: Reply[] }) {
    ...
    let template: HTMLTemplateElement | null = document.querySelector("#replyContainer4 template");
