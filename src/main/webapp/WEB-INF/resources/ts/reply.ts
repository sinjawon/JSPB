...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("click", (event) => {
        if (event.target && event.target.className == "deleteReply") {
            let replyNum = parseInt((event.target as HTMLElement).getAttribute("data-replynum") || "", 10);
...
function displayReply(replies: { replyList: Reply[] }) {
    ...
    let template: HTMLTemplateElement | null = document.querySelector("#replyContainer template");
