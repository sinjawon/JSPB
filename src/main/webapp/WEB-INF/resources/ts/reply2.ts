...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let addReplyBtn2 = document.getElementById("addReplyBtn2") as HTMLButtonElement;
    if (addReplyBtn2) {
    ...
    document.addEventListener("click", (event) => {
        if (event.target && event.target.className == "deleteReply2") {
            let replyNum2 = parseInt((event.target as HTMLElement).getAttribute("data-replynum") || "", 10);
...
async function loadReply2() {
    try {
        let boardListNumInput2 = document.getElementById("boardListNumInput2") as HTMLInputElement;
        let boardListNum2 = boardListNumInput2.value;

        let formData = new FormData();
        formData.append("boardListNum2", boardListNum2);

        let response = await fetch(`/api/replylist2?${new URLSearchParams(formData)}`, {
            method: "GET",
            headers: {
...
function displayReply2(replies2: { replyList2: Reply[] }) {
    ...
    let template: HTMLTemplateElement | null = document.querySelector("#replyContainer2 template");
...
async function addReply2() {
    try {
        let formData = new FormData(document.getElementById("replyForm2") as HTMLFormElement);
...
function clearReplyInput2() {
    let replyContentsInput2 = document.getElementById("replyContents2") as HTMLTextAreaElement;
